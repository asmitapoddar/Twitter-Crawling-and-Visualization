...
tweets = searchTwitter("#picture", n=100)
tweetdf = twListToDF(tweets)
names = tweetdf$screenName
...
par(mar=c(5,6,4,1)+.1)
barplot(top20, border=NA, las=2, main="Top 20 most frequent terms", cex.main=1.5, font=2, horiz=TRUE, col='dark blue')
...
#-----------------------How often does is user @mentioned?----------------------
...
plt = ggplot(head(df_hash,40), aes(x = tag, y = freq)) + geom_bar(stat="identity", fill = "black")
...
print(paste0("User's location: ", user$location))
print(paste0("Number of times user links to external sites: ", length(URL), ' (', p ,'%',')'))
print(paste0("Top three most commonly used words: ", names(top20)[1], '(', top20[[1]], '), ', names(top20)[2], '(', top20[[2]], '), ',names(top20)[3], '(', top20[[3]], ')'))
